  ...
  element: (dom) => {
    ...
    if (dom.querySelector(".elInput")) {
      ...
      data.content = {
        ...
        required: element.getAttribute("class").includes("required1")
          ? "required1"
          : "required0",
      };
      return data;
    ...
    if (dom.querySelector(".elVideo")) {
      ...
      if (dom.getAttribute("data-video-type") === "youtube") {
        ...
        if (ampersandPosition !== -1) {
          bgImage = `https://img.youtube.com/vi/${id.substring(
            0,
            ampersandPosition
          )}/maxresdefault.jpg`;
        } else {
    ...
    if (dom.getAttribute("data-de-type") === "featureimage") {
      ...
      const image = element
        .querySelector(".elScreenshot_image img")
        .getBoundingClientRect();

      let headlineHTML = element.querySelector(".elScreenshot_text_headline")
        .innerHTML;
      if (element.querySelector(".elScreenshot_text_headline div")) {
        headlineHTML = element.querySelector(".elScreenshot_text_headline")
          .innerHTML;
      }

      let paragraphHTML = element.querySelector(".elScreenshot_text_body")
        .innerHTML;
      if (element.querySelector(".elScreenshot_text_body div")) {
        paragraphHTML = element.querySelector(".elScreenshot_text_body")
          .innerHTML;
      }
      data.content = {
        visible: app.checkVisibility(dom),
        ...
        headline_text: element.querySelector(".elScreenshot_text_headline")
          .textContent,
        paragraph: paragraphHTML,
        paragraph_text: element.querySelector(".elScreenshot_text_body")
          .textContent,
      };
      return data;
    }
    ...
    if (dom.getAttribute("data-de-type") === "listimg") {
      ...
      const image = pageDocument.querySelectorAll(
        `#${dom.id} .elBulletList li`
      )[0];
    ...
    if (dom.getAttribute("data-de-type") === "imagegallery2") {
      ...
      if (
        src ===
        "https://via.placeholder.com/350x150/e1e5e6/6d7b8b?text=Thumbnail"
      ) {
