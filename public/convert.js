const convert = (jsonSections) => {
  // console.clear()
  console.time("conversion");

  let pageDocument = document;
  if (app.iframeId) {
    pageDocument = document.querySelector(`iframe#${app.iframeId}`)
      .contentDocument;
  }

  const contentId = app.makeId();
  const popupId = app.makeId();
  const htmlComputedStyles = getComputedStyle(
    pageDocument.querySelector("html")
  );
  const videoBg = pageDocument.querySelector(".modalBackdropWrapper");
  const backgroundParams = {};
  const backgroundColor = htmlComputedStyles.getPropertyValue(
    "background-color"
  );
  const backgroundImage = htmlComputedStyles.getPropertyValue(
    "background-image"
  );

  if (backgroundColor)
    backgroundParams["--style-background-color"] = backgroundColor;
  if (backgroundImage) {
  ...
  if (
    ...
    backgroundParams["--style-video-bg-url"] = videoBg.getAttribute(
    ...
    switch (endaction) {
      ...
      case "redirect":
        ...
        backgroundParams[
          "--style-video-bg-redirect-url"
        ] = videoBg.getAttribute("data-youtube-redirecturl");
        break;
    }
  }
  ...
  const backgroundAttrs = {
    ...
    "data-skip-background-video-settings": videoBg.getAttribute(
  ...
  const backgroundPosition = htmlComputedStyles.getPropertyValue(
    "background-position"
  );
  ...
  if (backgroundPosition) {
    ...
    backgroundAttrs["style"][
      "background-position"
    ] = `${data.horizontal} ${data.vertical} !important`;
  }
  ...
  const convertedJSON = {
    ...
    settings: null,
    popup: { ...popup(jsonSections, popupId) },
  };

  convertedJSON.content.children = sections(jsonSections, contentId);
